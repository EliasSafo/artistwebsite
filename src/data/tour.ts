...
export const upcomingTourDates = fetchTourDates().then(tourDates =>
  tourDates.filter(date => new Date(date.date) >= new Date())
    .sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime())
);

export const pastTourDates = fetchTourDates().then(tourDates =>
  tourDates.filter(date => new Date(date.date) < new Date())
